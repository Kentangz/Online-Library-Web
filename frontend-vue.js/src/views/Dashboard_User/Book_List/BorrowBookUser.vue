<template>
      ...
      <div v-if="book" class="row justify-content-center">
        <div class="col-md-6">
          <form @submit.prevent="borrowBook">
            ...
            <button type="button" class="btn btn-secondary w-100 shadow-sm py-2 mt-3" @click="cancelBorrowing">
              Cancel
            </button>
          </form>
        </div>
  ...
  export default {
    ...
    methods: {
      ...
      // Handle cancel borrowing
      cancelBorrowing() {
        this.borrowDate = ""; // Clear the borrow date
        this.errorMessage = ""; // Clear any error message
        this.successMessage = ""; // Clear any success message
        this.$router.push({ name: 'BookListUser' });
      }
    },
  };
  </script>
